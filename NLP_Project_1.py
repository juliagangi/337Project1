...
def build_json(data):
    ...
    award_names = get_keywords(award_names)
...
def build_human_readable(data):
    ...
    print('Host: '+hosts+'\n')
    award_names = get_awards(data)import json
import numpy as np
import csv
import spacy
from spacy import displacy
...
#awards = plus(awards)
#print(awards)
#for award in noms_input:
    #get_nominees(award, the_movies, the_shows)    for award in award_names:
        nominees = (get_nominees(award,the_movies,the_shows))
        nominees = ', '.join(nominees)
        presenters = ', '.join(get_presenters(award))
        winner = nominees[0]
